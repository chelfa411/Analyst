...
def get_dataframe(page):
    ...
    if start_index < len(current_search_results):
        ...
        return pd.DataFrame(columns=['index', 'title'])
...
with gr.Blocks() as demo:
    ...
    with gr.Row():
        drop_file = gr.Dropdown(list(file_choices.keys()), label="Choose the articles to explore", show_label=True)
        previous_button = gr.Button("Previous")
        next_button = gr.Button("Next")
    df = gr.Dataframe(value=get_dataframe(0), label="Articles")
    index_input = gr.Number(label="Enter the global index of the article:", minimum=0, maximum=len(articles)-1, step=1)
    ...
    drop_file.change(fn=update_articles, inputs=drop_file, outputs=df)
